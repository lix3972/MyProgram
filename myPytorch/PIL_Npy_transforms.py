...
A_path='a.jpg'
B_path='a.npy'
scipy.misc.imsave(A_path, a_npy.transpose(1,2,0))  # a_npy(3*5*6)保存为jpg图像 宽度6,高度5
np.save(B_path, b_npy)
# =================读取文件===之前a,之后A==========================================================
A_img = Image.open(A_path).convert('RGB')  # 6*5  w*h
A_img_ski = read_image(A_path)  # 3*5*6   3*h*w
B_npy = read_npy(B_path)  # 3*5*6                 3*h*w
# ================ crop 2行4列 h=2,w=4  ================================================
    # 方法1 先切再转换为tensor----------------------------------------
...
B_npy_crop = Lcrop.LocationCropNpy((2, 4))(B_npy, x, y)  # 3*h*w  3*2*4  切出的值 12 13 14 15; 22 23 24 25
