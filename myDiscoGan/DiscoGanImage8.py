...
cuda=True
# BATCH_SIZE = 1
w_NoP=2  # weight of people's number in the loss
...
# ===训练==训练==训练===训练==训练===================================================================================
    for i in range(9):
        ...
        # number of people Loss
        gen_loss_NoP = recon_criterion(torch.sum(AB), torch.sum(B))

        gen_loss = gen_loss_A_total + gen_loss_B_total + w_NoP*gen_loss_NoP
    ...
    if epoch == epochNum-1:
        torch.save(G_A, 'models/G_A{}.pkl'.format(epoch))
        torch.save(G_B, 'models/G_B{}.pkl'.format(epoch))
        torch.save(D_A, 'models/D_A{}.pkl'.format(epoch))
        torch.save(D_B, 'models/D_B{}.pkl'.format(epoch))
    elif epoch % 1000 == 1000-1:
        torch.save(G_A, 'models/G_A{}.pkl'.format(epoch))
        torch.save(G_B, 'models/G_B{}.pkl'.format(epoch))
        torch.save(D_A, 'models/D_A{}.pkl'.format(epoch))
        torch.save(D_B, 'models/D_B{}.pkl'.format(epoch))
    ...
    # if epoch % 1000 == 0:  # 1000-1:
        ...
        np.save('trainResults/' + str(int(epoch)) + '_AB.npy', AB_val.astype(np.float64)[:, :])
        scipy.misc.imsave('trainResults/' + str(int(epoch)) + '_ABA.jpg', ABA_val.astype(np.uint8)[:, :, :])
        np.save('trainResults/' + str(int(epoch)) + '_BAB.npy', BAB_val.astype(np.float64)[:, :])
