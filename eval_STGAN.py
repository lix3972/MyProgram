...
###########
	...
	path = "dataset"
	glasses0 = np.load("{0}/glasses.npy".format(path))
	upMove=x
	leftMove=y
	glasses1=np.concatenate([glasses0[0:10,upMove:144,:,:],glasses0[0:10,0:upMove,:,:]],axis=1)
	glasses=np.concatenate([glasses1[:,:,leftMove:144,:],glasses1[:,:,0:leftMove,:]],axis=2)
	...
	with tf.Session(config=tfConfig) as sess:
		...
		for b in range(opt.batchSize):
			util.imsave("eval_{0}/image_g{1}_input{2}{3}.png".format(opt.loadGP,b,x,y),ic0[b])
			util.imsave("eval_{0}/image_g{1}_output{2}{3}.png".format(opt.loadGP,b,x,y),icf[b])
